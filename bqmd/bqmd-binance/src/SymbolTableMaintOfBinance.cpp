/*!
 * \file SymbolTableMaintOfBinance.cpp
 * \project BetterQuant
 *
 * \author byrnexu
 * \date 2022/09/08
...
SymbolTableMaintOfBinance::convertSymbolTableFromExch(
  ...
  for (std::size_t i = 0; i < (*doc)["symbols"].Size(); ++i) {
    ...
    //! 这里可能会同时返回币本位永续和币本位交割，或者U本位永续和U本位交割，所以
    //! 需要跳过和当前配置不符的symbolType
...
SymbolTableMaintOfBinance::doMakeSymbolInfo(const Val& v) const {
  ...
  //! 通过alias确定当前symbolType是币本位、U本位或者现货
