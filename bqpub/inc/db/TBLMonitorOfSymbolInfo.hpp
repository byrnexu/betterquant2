...
class TBLMonitorOfSymbolInfo : public TBLMonitor<TBLSymbolInfo> {
  ...
  struct KeyMarketCodeSymbolCode
      : boost::multi_index::composite_key<
            symbolInfo::Record,
            MIDX_MEMBER(symbolInfo::Record, std::uint64_t, hashOfMktSym_)> {};

  struct KeyMarketCodeSymbolTypeExchSymbolCode
      : boost::multi_index::composite_key<
            symbolInfo::Record, MIDX_MEMBER(symbolInfo::Record, std::uint64_t,
 ...
 public:
  std::tuple<int, RecSymbolInfoSPtr> getRecSymbolInfoBySymbolCode(
    ...
    LOG_W("Can not find symbol info of symbol {} - {}. hash: {}", marketCode,
          symbolCode, hash);
  ...
  std::tuple<int, RecSymbolInfoSPtr> getRecSymbolInfoByExchSymbolCode(
    ...
    LOG_W("Can not find symbol info of exch symbol {} - {} - {}. hash: {}",
          marketCode, symbolType, exchSymbolCode, hash);
  ...
  void initNecessaryDataStructures(
    ...
    std::size_t numOfSymbol = 0;
    {
      ...
      for (const auto& tblRec : *tblRecSet) {
        ...
        LOG_D(fmt::format(
            "Add rec {} - {} to tbl symbol info of cache. "
            "hashOfMktSym: {}, hashOfMktSymExchSym: {}",
            symbolInfo->marketCode, symbolInfo->symbolCode,
            symbolInfo->hashOfMktSym_, symbolInfo->hashOfMktSymExchSym_));
      }
