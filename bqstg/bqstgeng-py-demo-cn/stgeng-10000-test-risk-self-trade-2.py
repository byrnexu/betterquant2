...
class StgInstTaskHandler(StgInstTaskHandlerBase):
    def __init__(self, stg_eng):
        ...
        self.status_code53 = -1
        self.status_code54 = -1
        ...
        self.order_id63 = 0
        self.order_id64 = 0
        self.status_code61 = -1
        self.status_code62 = -1
        self.status_code63 = -1
        self.status_code64 = -1
    ...
    def on_stg_start(self):
        ...
        # 先用100002下一个卖单挂单，再下一个买开和买平全成，但是价格都高于卖单，但是因为100002没有设置自成交风控，所以不会触发自成交
        ts += 1       ; self.stg_eng.install_stg_inst_timer( stg_inst_id=1, timer_name="DBUpdate6", exec_at_startup=ExecAtStartup.IsFalse, millisec_interval=1000 * ts, max_exec_times=1)
        ts += interval; self.stg_eng.install_stg_inst_timer( stg_inst_id=1, timer_name="Test6", exec_at_startup=ExecAtStartup.IsFalse, millisec_interval=1000 * ts, max_exec_times=1)
        ts += 1       ; self.stg_eng.install_stg_inst_timer( stg_inst_id=1, timer_name="DBRestore6", exec_at_startup=ExecAtStartup.IsFalse, millisec_interval=1000 * ts, max_exec_times=1)
    ...
    def on_stg_inst_timer(self, stg_inst_info, timer_name):
        if stg_inst_info.stg_inst_id == 1:
            ...
            if timer_name == "DBRestore5":
                self.DBRestore5(stg_inst_info)

            # 先用100002下一个卖单挂单，再下一个买开和买平全成，但是价格都高于卖单，但是因为100002没有设置自成交风控，所以不会触发自成交
            if timer_name == "DBUpdate6":
                self.DBUpdate6(stg_inst_info)
            if timer_name == "Test6":
                self.Test6(stg_inst_info)
            if timer_name == "DBRestore6":
                self.DBRestore6(stg_inst_info)
    ...
    def Test3(self, stg_inst_info):
        ...
        ret_of_order34 = self.stg_eng.order(stg_inst_info, market_code=MarketCode.DCE, symbol_code="a2401", side=Side.Ask, pos_direction=PosDirection.Open, order_price=100, order_size=101, trd_acct_id=100001, close_tday_stg=CloseTDayStg.AllowCloseTDay, algo_id=0, simed_td_info=simed_td_info)
        # fmt: on
    ...
    def Test4(self, stg_inst_info):
        ...
        ret_of_order44 = self.stg_eng.order(stg_inst_info, market_code=MarketCode.DCE, symbol_code="a2401", side=Side.Bid, pos_direction=PosDirection.Close, order_price=100, order_size=101, trd_acct_id=100001, close_tday_stg=CloseTDayStg.AllowCloseTDay, algo_id=0, simed_td_info=simed_td_info)
        # fmt: on
    ...
    def Test5(self, stg_inst_info):
        ...
        simed_td_info.order_status = OrderStatus.ConfirmedByExch
        # 第一个是挂单
        ret_of_order51 = self.stg_eng.order(stg_inst_info, market_code=MarketCode.DCE, symbol_code="a2403", side=Side.Bid, pos_direction=PosDirection.Open, order_price=100, order_size=101, trd_acct_id=100002, close_tday_stg=CloseTDayStg.AllowCloseTDay, algo_id=0, simed_td_info=simed_td_info)

        simed_td_info.order_status = OrderStatus.Filled
        simed_td_info.trans_detail_group.append(TransDetail(slippage=0, filled_per=1, ld=LiquidityDirection.Maker))
        # 第二第三个是全成的订单
        ret_of_order52 = self.stg_eng.order(stg_inst_info, market_code=MarketCode.DCE, symbol_code="a2403", side=Side.Ask, pos_direction=PosDirection.Open, order_price=100, order_size=101, trd_acct_id=100002, close_tday_stg=CloseTDayStg.AllowCloseTDay, algo_id=0, simed_td_info=simed_td_info)
        ret_of_order53 = self.stg_eng.order(stg_inst_info, market_code=MarketCode.DCE, symbol_code="a2403", side=Side.Ask, pos_direction=PosDirection.Close, order_price=99, order_size=101, trd_acct_id=100002, close_tday_stg=CloseTDayStg.AllowCloseTDay, algo_id=0, simed_td_info=simed_td_info)
        ret_of_order54 = self.stg_eng.order(stg_inst_info, market_code=MarketCode.DCE, symbol_code="a2401", side=Side.Ask, pos_direction=PosDirection.Open, order_price=100, order_size=101, trd_acct_id=100002, close_tday_stg=CloseTDayStg.AllowCloseTDay, algo_id=0, simed_td_info=simed_td_info)
        # fmt: on
        ...
        self.order_id55 = ret_of_order53[1]
        mylogger.info(f"===> send order {self.order_id53}")
        self.order_id54 = ret_of_order54[1]
        mylogger.info(f"===> send order {self.order_id54}")

        # 撤销订单，避免对后续测试产生影响
        self.stg_eng.cancel_order(self.order_id51)
        return
    ...
    # 先用100002下一个卖单挂单，再下一个买开和买平全成，但是价格都高于卖单，但是因为100002没有设置自成交风控，所以不会触发自成交
    def DBUpdate6(self, stg_inst_info):
        mylogger.debug("===================== BEGIN TO TEST6")
        # 修改限制
        self.stg_eng.sync_exec_sql(
            "INSERT INTO `riskSelfTradeCtrlRange`(`step`, `condition`, `name`) VALUES('acctId', 'acctId=10001', 'acctId=10001');"
        )
    ...
    # 先用100002下一个卖单挂单，再下一个买开和买平全成，但是价格都高于卖单，但是因为100002没有设置自成交风控，所以不会触发自成交
    def Test6(self, stg_inst_info):
        # fmt: off
        simed_td_info = SimedTDInfo()
        simed_td_info.order_status = OrderStatus.ConfirmedByExch
        # 第一个是挂单
        ret_of_order61 = self.stg_eng.order(stg_inst_info, market_code=MarketCode.DCE, symbol_code="a2403", side=Side.Ask, pos_direction=PosDirection.Open, order_price=100, order_size=101, trd_acct_id=100002, close_tday_stg=CloseTDayStg.AllowCloseTDay, algo_id=0, simed_td_info=simed_td_info)

        simed_td_info.order_status = OrderStatus.Filled
        simed_td_info.trans_detail_group.append(TransDetail(slippage=0, filled_per=1, ld=LiquidityDirection.Maker))
        # 第二第三个是全成的订单
        ret_of_order62 = self.stg_eng.order(stg_inst_info, market_code=MarketCode.DCE, symbol_code="a2403", side=Side.Bid, pos_direction=PosDirection.Open, order_price=100, order_size=101, trd_acct_id=100002, close_tday_stg=CloseTDayStg.AllowCloseTDay, algo_id=0, simed_td_info=simed_td_info)
        ret_of_order63 = self.stg_eng.order(stg_inst_info, market_code=MarketCode.DCE, symbol_code="a2403", side=Side.Bid, pos_direction=PosDirection.Close, order_price=101, order_size=101, trd_acct_id=100002, close_tday_stg=CloseTDayStg.AllowCloseTDay, algo_id=0, simed_td_info=simed_td_info)
        ret_of_order64 = self.stg_eng.order(stg_inst_info, market_code=MarketCode.DCE, symbol_code="a2401", side=Side.Bid, pos_direction=PosDirection.Close, order_price=100, order_size=101, trd_acct_id=100002, close_tday_stg=CloseTDayStg.AllowCloseTDay, algo_id=0, simed_td_info=simed_td_info)
        # fmt: on

        self.order_id61 = ret_of_order61[1]
        mylogger.info(f"===> send order {self.order_id61}")
        self.order_id62 = ret_of_order62[1]
        mylogger.info(f"===> send order {self.order_id62}")
        self.order_id63 = ret_of_order63[1]
        mylogger.info(f"===> send order {self.order_id63}")
        self.order_id64 = ret_of_order64[1]
        mylogger.info(f"===> send order {self.order_id64}")

        # 撤销订单，避免对后续测试产生影响
        self.stg_eng.cancel_order(self.order_id61)
        return

    # 先用100002下一个卖单挂单，再下一个买开和买平全成，但是价格都高于卖单，但是因为100002没有设置自成交风控，所以不会触发自成交
    def DBRestore6(self, stg_inst_info):
        self.stg_eng.sync_exec_sql(
            "DELETE FROM `riskSelfTradeCtrlRange` WHERE step = 'acctId' AND `condition` = 'acctId=10001' AND `name` = 'acctId=10001';"
        )
    ...
    def on_order_ret(self, stg_inst_info, order_info):
        ...
        # 先用100001下一个卖单挂单，再下一个买开和买平全成，但是价格都低于卖单，也就是不会触发自成交
        if order_info.order_id == self.order_id51:
            self.status_code51 = order_info.status_code
            if order_info.status_code != 0:
                mylogger.error(f"invalid status code of order {order_info.status_code}")

        if order_info.order_id == self.order_id52:
            self.status_code52 = order_info.status_code
            if order_info.status_code != 0:
                mylogger.error(f"invalid status code of order {order_info.status_code}")

        if order_info.order_id == self.order_id53:
            self.status_code53 = order_info.status_code
            if order_info.status_code != 0:
                mylogger.error(f"invalid status code of order {order_info.status_code}")

        if order_info.order_id == self.order_id54:
            self.status_code54 = order_info.status_code
            if order_info.status_code != 0:
                mylogger.error(f"invalid status code of order {order_info.status_code}")

        # 先用100001下一个买单挂单，再下一个卖开和卖平全成，但是价格都高于买单，也就是不会触发自成交
        if order_info.order_id == self.order_id61:
            self.status_code61 = order_info.status_code
            if order_info.status_code != 0:
                mylogger.error(f"invalid status code of order {order_info.status_code}")

        if order_info.order_id == self.order_id62:
            self.status_code62 = order_info.status_code
            if order_info.status_code != 0:
                mylogger.error(f"invalid status code of order {order_info.status_code}")

        if order_info.order_id == self.order_id63:
            self.status_code63 = order_info.status_code
            if order_info.status_code != 0:
                mylogger.error(f"invalid status code of order {order_info.status_code}")

        if order_info.order_id == self.order_id64:
            self.status_code64 = order_info.status_code
            if order_info.status_code != 0:
                mylogger.error(f"invalid status code of order {order_info.status_code}")
