...
//! 每当帐户余额发生更改时，都会发送一个事件outboundAccountPosition，其中包含可能
//! 由生成余额变动的事件而变动的资产。
//!
//! 转出100usdc到合约账户的2个事件内容。
//!
//! {"e":"balanceUpdate","E":1657061859702,"a":"USDC","d":"-100.00000000","T":1657061859701}
//!
//! {"e":"outboundAccountPosition","E":1657061859702,"u":1657061859701,"B":
//! [{"a":"USDC","f":"412.66932800","l":"0.00000000"}]
//! }
//!
//! 转出所有usdc到合约账户后收到的outboundAccountPosition
//! {"e":"outboundAccountPosition","E":1657330349863,"u":1657330349863,"B":
//! [{"a":"USDC","f":"0.00000000","l":"0.00000000"}]
//! }
//!
//! 下单的时候会收到以下余额变动事件
//! {"e":"outboundAccountPosition","E":1657063273450,"u":1657063273449,"B":
//! [
//!   {"a":"BNB" ,"f":"0.00001061"  ,"l":"0.00000000"  },
//!   {"a":"USDT","f":"0.39080000"  ,"l":"300.03000000"},
//!   {"a":"USDC","f":"395.66932800","l":"0.00000000"  }
//! ]
//! }
WSCliAsyncTaskArgSPtr WSCliOfExchBinance::MakeWSCliAsyncTaskArg(
  ...
  if (yyjson_equals_str(valE, "outboundAccountPosition")) {
    ...
    //! executionReport 是现货委托回报
    //! ORDER_TRADE_UPDATE 是U本位和币本位委托回报
    LOG_D("Recv {}", payload);
...
OrderInfoSPtr WSCliOfExchBinance::makeOrderInfoFromExchOfSpot(
  ...
  if (strcmp(exchOrderStatus, "CANCELED") != 0) {
    //! 外部订单的clientOrderId为字符串，直接转换为整型会crash
    orderInfoFromExch->orderId_ = getHashIfIsNotNum(yyjson_get_str(valc));
  } else {
    orderInfoFromExch->orderId_ = getHashIfIsNotNum(yyjson_get_str(valC));
  }

  strncpy(orderInfoFromExch->exchOrderId_,
          fmt::format("{}", yyjson_get_sint(vali)).c_str(),
          sizeof(orderInfoFromExch->exchOrderId_) - 1);
...
//! 以下按顺序分别为 单向逐仓 单向全仓 双向全仓 的 订单确认和全成
...
OrderInfoSPtr WSCliOfExchBinance::makeOrderInfoFromExchOfContracts(
  ...
  strncpy(orderInfoFromExch->exchOrderId_,
          fmt::format("{}", yyjson_get_uint(vali)).c_str(),
          sizeof(orderInfoFromExch->exchOrderId_) - 1);
  if (valn) {
    orderInfoFromExch->fee_ = CONV(Decimal, yyjson_get_str(valn));
  }
