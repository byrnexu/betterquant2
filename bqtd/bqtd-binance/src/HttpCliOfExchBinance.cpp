...
//! 下单失败应答在基类中发送，这里只需要说明什么样的应答是失败即可
...
//! 撤单失败应答在基类中发送，这里只需要说明什么样的应答是失败即可
...
//! 同步获取listenKey，基类中进行websocket连接需要listenKey
...
//! 定时延长listenKey生命周期，确保websocket连接不断开
...
OrderInfoSPtr HttpCliOfExchBinance::makeOrderInfoFromExchOfSpot(
  ...
  if (valCode && valMsg) {
    ...
    //! 因为获取OrdMgr中原始订单的查询条件是marketCode + exchSymbolCode，所以
    //! 必须保存marketCode
    ret->marketCode_ = tdSvc_->getMarketCodeEnum();
    ret->orderId_ = orderInfoInOrdMgr->orderId_;
    strcpy(ret->exchOrderId_, orderInfoInOrdMgr->exchOrderId_);
    ...
    strncpy(ret->exchOrderId_,
            fmt::format("{}", yyjson_get_sint(valOrderId)).c_str(),
            sizeof(ret->exchOrderId_) - 1);
...
void HttpCliOfExchBinance::doTestOrderOfUBasedContracts() {
  ...
  orderInfo->posDirection_ = PosDirection::Open;
