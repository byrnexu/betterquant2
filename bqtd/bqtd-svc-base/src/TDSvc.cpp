...
int TDSvc::doInit() {
  ...
  //! appName_ = TD-Binance-Spot-10000
...
void TDSvc::initAssetsMgr() {
  ...
  const auto sql = fmt::format(
      ...
      TBLAssetInfo::TableName, getAcctId(), getMarketCode(), getSymbolType());
...
void TDSvc::initOrdMgr() {
  ...
  const auto sql = fmt::format(
      "SELECT * FROM {} WHERE `orderStatus` < {} AND `marketCode` = "
      "'{}' AND `symbolType` = '{}' AND `acctId` = {}; ",
      TBLOrderInfo::TableName, filled, getMarketCode(), getSymbolType(),
      getAcctId());
...
void TDSvc::initSHMCliOfTDSrv() {
  ...
  //! addr = "TD-Binance-Spot-10000@TD@TDGWChannel@Trade"
  const auto addr =
...
void TDSvc::initSHMCliOfRiskMgr() {
  ...
  //! addr = "TD-Binance-Spot-10000@RISK@TDGWChannel@Trade"
  const auto addr =
