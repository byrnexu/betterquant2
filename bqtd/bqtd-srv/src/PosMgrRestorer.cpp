...
int PosMgrRestorer::exec() {
  //! 获取acctId及其最后参与计算仓位的lastNoUsedToCalcPos
...
PosMgrRestorer::queryAllAcctId() {
  ...
  const auto sql = fmt::format(R"(SELECT * FROM {};)", TBLAcctInfo::TableName);
...
PosMgrRestorer::queryAcctId2lastNoUsedToCalcPos() {
  ...
  const auto sql = fmt::format(
      "SELECT * FROM {} AS a WHERE `lastNoUsedToCalcPos` = "
      "(SELECT MAX(b.`lastNoUsedToCalcPos`) FROM {} AS b "
      "WHERE a.`acctId` = b.`acctId`)",
      TBLPosInfo::TableName, TBLPosInfo::TableName);
...
PosMgrRestorer::getAcct2OrderInfoForPosMgrNotCalc(
  ...
  for (const auto rec : acctId2lastNoUsedToCalcPos) {
    ...
    const auto sql = fmt::format(
        "SELECT * FROM {} WHERE acctId = {} and noUsedToCalcPos > {};",
        TBLTradeInfo::TableName, acctId, lastNoUsedToCalcPos);
    ...
    //!
    //! 因为生成的tblRecSet的顺序是按照JsonStrOfKeyFields排序的，所以使用
    //! std::map<std::uint64_t, OrderInfoSPtr> 确保顺序。
    //!
    for (const auto& tblRec : *tblRecSet) {
  ...
  for (const auto& rec : acctId2OrderInfoForPosNotCalc) {
    ...
    for (const auto& rec : no2OrderInfo) {
      ...
      const auto posChgInfo =
          posMgr->updateByOrderInfoFromTDGW<LockFunc::False>(orderInfo);
