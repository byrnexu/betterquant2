...
bool FlowCtrlSvc::exceedFlowCtrl(const std::string& taskName,
...

  {
    ...
    //! 获取当前组的流控信息（包括时间长度，限制数量和timePointGroupOfTaskHasBeenExec_）
    const auto iter = groupNo2FlowLimitInfo_.find(groupNo);
    auto& flowLimitInfo = iter->second;

    //! 当最初任务数量不足flowLimitInfo->limitNum_的时候，肯定不超流控
    ...
    if (DEC::GE(taskNumPerMSIfExec, taskNumLimitPerMS)) {
