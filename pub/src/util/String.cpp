...
bool HasInvisibleCharImpl(std::string_view str) {
  for (char c : str) {
  ...
  return false;  // 不包含不可见字符
}

bool HasInvisibleChar(const std::string& str) {
  return HasInvisibleCharImpl(str);
}

bool HasInvisibleChar(std::string_view str) {
  return HasInvisibleCharImpl(str);
}

bool IsValidJsonStrVal(const std::string& str) {
  //! 考虑str包含双引号的情况，所以必须对str进行escape
  const auto tmp = fmt::format(R"({{"val":"{}"}})", EscapeStr(str));
  Doc doc;
  doc.Parse(tmp.data());
