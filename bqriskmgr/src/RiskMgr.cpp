...
int RiskMgr::doInit() {
  ...
  subMgr_ = std::make_shared<SubMgr>(
      AppName, [this](const auto shmBuf, auto shmBufLen) {
        ...
        if (header->msgId_ == MSG_ID_ON_MD_TICKERS) {
          const auto tickers = MakeMsgSPtrByTask<Tickers>(task);
          marketDataCache_->cache(tickers);
        }
      });
...
void RiskMgr::initPosMgr() {
  posMgr_ = std::make_shared<RiskPosMgr>();
...
void RiskMgr::initAssetsMgr() {
  ...
  const auto sql = fmt::format("SELECT * FROM {}", TBLAssetInfo::TableName);
  assetsMgr_->init(CONFIG, getDBEng(), sql);
}

void RiskMgr::initOrdMgr() {
  ordMgr_ = std::make_shared<RiskOrdMgr>();
...
int RiskMgr::initRiskMgrTaskDispatcher() {
  ...
  //! 除了OrderInfo之外其他类型的消息返回acctId=0，也就是由0号线程处理
...
void RiskMgr::initScheduleTaskBundle() {
  ...
  const auto milliSecIntervalOfPubPosSnapshotOfAll =
      CONFIG["milliSecIntervalOfPubPosSnapshotOfAll"].as<std::uint32_t>();
  getScheduleTaskBundle()->emplace_back(std::make_shared<ScheduleTask>(
      "pubPosSnapshotOfAll",
      [this]() {
        getPubSvc()->pubPosSnapshotOfAll();
        return true;
      },
      ExecAtStartup::True, milliSecIntervalOfPubPosSnapshotOfAll));
